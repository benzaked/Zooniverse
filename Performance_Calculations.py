...
old_users = pd.read_csv('user_project_matrix.csv')['user'].values
...
def get_precision_and_recall_by_time_split(user, k, algorithm, ip_address):
    ...
    if len(projects_list)>= Recommender.HISTORY_THRES:
        ...
        with open('other_algorithms)recs.txt', 'a')  as f:
            f.write(str(user) + " " + str(relevant_projects) + " " + str(known_user_likes_test)+ "\n")
